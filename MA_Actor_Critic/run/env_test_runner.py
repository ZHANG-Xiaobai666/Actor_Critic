    ...
    def run(self):
        ...
        for episode in range(self.episodes):
            ...
            for step in range(self.episode_length):
                ...
                _, actions = self.collect(obs, deterministic=True)             # collect actions and corresponding probs
    ...
    def collect(self, obs, deterministic=False):
        ...
        if deterministic:
            for agent in range(int(self.num_agent)):
                prob, action = self.agents[agent].select_action(obs[agent], deterministic=True)
